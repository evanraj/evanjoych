	...
	if(isset($_POST['action']) AND $_POST['action'] =='submit_pc') {
		...
		foreach ($_POST['group_team'] as $group_val ) {
	 		$gaming_pc_data 		= array(
	 				...
	 				'pc_gaming_type' 	=> $group_val['gaming_type'],
	 			);
	...
	if(isset($_POST['action']) AND $_POST['action'] =='update_pc') {
		...
		foreach ($_POST['group_team'] as $group_val ) {
	 		$gaming_pc_data 		= array(
	 				...
	 				'pc_category' 		=> $group_val['category'],
	 				'pc_gaming_type' 	=> $group_val['gaming_type'],
	 				// 'pc_hours' 			=> $group_val['gaming_type'],
	 				// 'pc_price' 			=> $group_val['gaming_type'],
	 			);
	 		$wpdb->insert( $system_table, $gaming_pc_data);
		}
	}
				...
				<div class="retail-repeater group_team">
				  	<div data-repeater-list="group_team" class="div-table">
					    <div class="div-table-row">
						    ...
						    <div class="div-table-head ">Gaming Type</div>						   
						</div>
						...
						<div data-repeater-item class="repeterin div-table-row">
						    ...
						    <div class="div-table-col">
	                            ...
	                            <select name="category">
	                            	...
	                            	?>
								  	<option value="1"<?php if($cat == '1'){ echo 'selected';} ?>>PS4</option>
								  	<option value="2"<?php if($cat == '2'){ echo 'selected';} ?>>PS2</option>
								  	<option value="3"<?php if($cat == '3'){ echo 'selected';} ?>>VR</option>
								  	<option value="4"<?php if($cat == '4'){ echo 'selected';} ?>>Xbox</option>
								</select>	
	                            </div>
						    </div> 
						     <div class="div-table-col">
	                            <div class="hours player">
	                            <select name="gaming_type">
	                            	<?php 
	                            	if($update_system_data){
	                            		$type = $update_system_data->pc_gaming_type;
	                            	}
	                            	else {
	                            		$type = '';
	                            	}
	                            	...
	                            	?>
								  	<option value="console"<?php if($type == 'console'){ echo 'selected';} ?>>Console Gaming</option>
								  	<option value="vr"<?php if($type == 'vr'){ echo 'selected';} ?>>Virtual Gaming</option>
								  	<option value="motion_sensor"<?php if($type == 'motion_sensor'){ echo 'selected';} ?>>Motion Sensor</option>
								  <!-- 	<option value="augumented"<?php if($type == 'augumented'){ echo 'selected';} ?>>Augumented Reality</option> -->
								</select>	
	                            </div>
						    </div> 
				        </div>
