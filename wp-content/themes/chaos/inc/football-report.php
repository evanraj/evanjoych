...
(case when member_table.membership_no is null then 0 else member_table.membership_no end) as  membership_no,
    ...
    from 
(
  select id,member_id,ft_bill_no,ft_date,ft_played_hours,ft_member_name,ft_member_phone_number,ft_total,ft_gst,ft_member_discount,ft_discount,created_by,was_bulid,ft_football_bill,ft_member_discount_value,ft_discount_value,ft_gst_value from {$football_table} WHERE active = 1 and was_bulid = 1 and ft_bill_date >='{$timestamp1}' and ft_bill_date <='{$timestamp2}'
  ) as bill 
left join
  (
...
left join 
(select inv_id,sum(hours) as hours,price_per_hour,sum(price) as price from {$billing_detail_table} WHERE active = 1 GROUP by inv_id ) as  football_details
...
<?php
		...
		$subject = 'Football Daily Report - '.date("d-m-Y");
		

 
	$message1 = '<html><body>
			<h1 style="margin-top:30px;margin-bottom:0px;"><b>Football Daily Report</b></h1>
				<table border="1">
					<thead>
						<tr>
							<th>S.No</th>
					...
					<tbody>'; 
						...
						foreach($pricing_football as $played_detail){
                            ...
                            <td>".$played_detail->ft_bill_no."</td> 
                            <td>".$played_detail->ft_member_name."</td>
                            <td>".$played_detail->ft_member_phone_number."</td>
                            <td>".$played_detail->membership_no."</td>
                            <td>".$played_detail->ft_played_hours."</td>
                            <td>".$played_detail->hours."</td>
                            <td>".$played_detail->price_per_hour."</td>
                            <td>".$played_detail->price."</td>
                            <td>".$played_detail->ft_discount_value."</td>
                            <td>".$played_detail->ft_member_discount_value."</td>
                            <td>".$played_detail->ft_gst_value."</td>
                            <td>".$played_detail->ft_football_bill."</td></tr>";
