 ...
 */
  	...
  	global $wpdb; 
    $football_table           = $wpdb->prefix.'chaos_lazertag_billing';
    ...
    from (
    SELECT * FROM {$football_table} WHERE was_bulid =1 and active =1 and lazer_date >='{$timestamp1}' and lazer_date <='{$timestamp2}' )  as billing
   left join 
   (
       select id as mem_id,membership_no,first_name,phone from {$member_table} WHERE active = 1 
   ) as member_table 
  on billing.member_id= member_table.mem_id ";
   	

   	$pricing_lazer   	= $wpdb->get_results( $query_details, OBJECT ); 
...
/*body {
	...
	<?php 
			...
			$subject = 'Lazertag Daily Report - '.date("d-m-Y");

		$message1 = '<html><body>
				<h1 style="margin-top:30px;margin-bottom:0px;"><b>Lazertag Daily Report</b></h1>
				<table border="1">
					<thead>
						<tr>
							<th>S.No</th>
					...
					<tbody>';
						...
						$i=1;
						foreach($pricing_lazer as $played_detail){

					        $message2.="<tr><td>".$i."</td>
					        <td>".$played_detail->created_by."</td>
                            <td>".$played_detail->lazertag_bill_no."</td> 
                            <td>".$played_detail->lazertag_member_name."</td>
                            <td>".$played_detail->lazertag_member_phone_number."</td>
                            <td>".$played_detail->membership_no."</td>
                            <td>".$played_detail->lazertag_played_hours."</td>
                            <td>".$played_detail->lazertag_hours."</td>
                            <td>".$played_detail->lazertag_amount."</td>
                            <td>".$played_detail->lazertag_total."</td>
                            <td>".$played_detail->lazertag_discount_value."</td>
                            <td>".$played_detail->lazertag_member_discount_value."</td>
                            <td>".$played_detail->lazertag_gst_value."</td>
                            <td>".$played_detail->lazertag_bill."</td></tr>";
